...
Server.prototype.resizeImage = function (src, dest, parameters, callback) {
    ...
    fs.stat(src, function handleOriginalStat(err, size) {
        ...
        if (parameters.width) {
            if (parameters.width.toString() == parameters.size) {
                self.options.trace('resize_auto');
                imgr.resize(Number(parameters.size));
...
Server.prototype.compileRegexp = function (pattern) {
    ...
    pattern = pattern.replace(':size', '(\\d+|\\d+x|x+\\d+|\\d+x\\d+(?:-[a-z]+)?)')
                     .replace('/:path', '(?:/(.+?))?')
                     .replace(':ext', '([^/]+?)')
                     .replace(':file', '([^/]+?)');
    ...
    return function compileRegexp(url) {
        ...
        if (parsed.size) {
            ...
            dimensions = parsed.size.split('-', 2);
            result.orientation = dimensions[1] || null;
            dimensions = dimensions[0].split('x', 2);
            result.height = Number(dimensions[1]) || null;
            result.width = Number(dimensions[0]) || null;
