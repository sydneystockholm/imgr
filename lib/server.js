...
Server.prototype.middleware = function () {
    ...
    return function imgrMiddleware(request, response, next) {
        ...
        base_static(request, response, function imgrStage1(err) {
            if (err) {
                return next(err);
            }
            ...
            cache_static(request, response, function imgrStage2(err) {
                if (err) {
                    return next(err);
                }
                ...
                fs.stat(src_image, function imgrStage3(err, size) {
                    if (err | !size) {
                        trace('missing_original');
                        request.url = original_url;
                        return next();
                    }
                    ...
                    //Save the image
                    imgr.save(dest_image, function imgrStage4(err) {
                        if (err) {
                            trace('gm_fail');
                        ...
                        //..and serve it
                        cache_static(request, response, function imgrStage5(err) {
                            if (err) {
                                return next(err);
                            }
                            request.url = original_url;
